...
import {DEFAULT_SN_INSTANCE_ALIAS, SN_INSTANCE_ALIAS} from '../test_utils/test_config.js'
...
describe('live-test instance alias', () => {
  ...
  it('does not embed the live alias in session or CLI calls', () => {
    const offenders: string[] = []
    const liveLiteral = new RegExp(
      String.raw`(?:createBrowserSession\(\s*\{\s*alias:|'-a',\s*)['"\`]${DEFAULT_SN_INSTANCE_ALIAS}['"\`]`,
    )
