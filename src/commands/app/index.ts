...
export class App extends AuthenticatedCommand<typeof App> {

  static args = {
  }
static description = 'Manage ServiceNow applications: uninstall applications from your instance.\n\n' +
    'This command provides programmatic control over ServiceNow applications, allowing you to uninstall ' +
...
static flags = {
    ...
    uninstall: Flags.boolean({char: 'u', description: 'Uninstall the app', required: false}),
    
    applicationId: Flags.string({char: 'i', description: 'Application sys_id', required: false}),
    scope: Flags.string({char: 's', description: 'Scope of application.', required: false}),
  }

  // private _logger:Logger = new Logger("AppCommand");

  async run(): Promise<void> {
    const {args, flags} = await this.parse(App);

    this.log("App command");
